...
$buyersEnquiry->id = $_POST['enqId'];
$buyerData = $buyersEnquiry->getnotificationData();
...
if(mysqli_num_rows($buyerData) > 0)
{
    while($row = mysqli_fetch_assoc($buyerData))
    {
        ...
        $buyerRecords["entryDate"] = date("d-M-Y", strtotime($row["entryDate"]));
        ...
        $buyerRecords["remarks"] = $row["remarks"];

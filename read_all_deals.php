...
$vehicleData = $reports->readAllDeals();
...
if(mysqli_num_rows($vehicleData) > 0 )
{
    while($row = mysqli_fetch_assoc($vehicleData))
    {
        ...
        $dealArray['dealAmount'] = $row['dealAmount'];
        $dealArray['dealEntryDate'] = $row['dealEntryDate'];
        $dealArray['dueAmount'] = $row['dueAmount'];
        $dealArray['chequeNo'] = $row['chequeNo'];
